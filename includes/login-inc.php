...
if (isset($_POST['login-submit'])) {
  ...
  // Then we perform a bit of error handling to make sure we catch any errors made by the user. If we do run into an error we need to stop the rest of the script from running, and take the user back to the login form with an error message.
  ...
  if (empty($mailuid) || empty($password)) {
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      // If there is an error we send the user back to the registration page.
